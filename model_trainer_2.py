...
M_name = 'Models/testbrain_V0.6.2.keras'
M_ch_name = 'Models/testCH.bain_V0.6.2.keras'
names = []

#(x_train, y_train), (x_test, y_test) = mnist.load_data()
data = []
for i in os.listdir('foto_ok_2/'):
    img = cv2.imread('foto_ok_2/' + str(i))
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    data.append([img, [1, 0]])

np.random.shuffle(data)
data = data[:len(os.listdir('foto_bad_2/'))]
print(len(data))
for i in os.listdir('foto_bad_2/'):
    img = cv2.imread('foto_bad_2/' + str(i))
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    data.append([img, [0, 1]])
print(len(data))
np.random.shuffle(data)
...
##############################################
data_test=[]
for i in os.listdir('foto_ok_test_2/'):
    img = cv2.imread('foto_ok_test_2/' + str(i))
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    data_test.append([img, [1, 0]])
    names.append('ok_' + i)
print(len(data_test))
for i in os.listdir('foto_bad_test_2/'):
    img = cv2.imread('foto_bad_test_2/' + str(i))
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
callback = keras.callbacks.ModelCheckpoint(filepath=M_ch_name, save_best_only=True, mode='max', monitor="val_accuracy", verbose=1)
